...
observe the new configurations within a bounded but UNSPECIFIED time. If
software needs to ensure that such modifications have been observed by the I/O
MPT Checker, it must issue an `IOFENCE` after the modifications and wait for
successful completion. Successful completion of an `IOFENCE` operation also
guarantees that the I/O MPT Checker has observed all configuration updates made
prior to the `IOFENCE`. If the configuration updates modify the MPT mode and/or
PPN, an `MPTINVAL` operation with `PPNV` set to 0 must be issued before the
`IOFENCE`.
